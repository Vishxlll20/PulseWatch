...
const verifyOtp = async (req, res, next) => {
  ...
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ message: "Please request OTP first.", success: false });
    }
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (decoded.purpose !== "reset") {
      return res
        .status(401)
        .json({ message: "Invalid reset session.", success: false });
    }
...
const googleCallback = async (req, res, next) => {
  try {
    ...
    res.cookie("token", token, cookieOptions(7 * 24 * 60 * 60 * 1000));
    res.redirect(`${config.CLIENT_URL}/auth/google/callback`);
  } catch (error) {
    next(error);
  }
};
